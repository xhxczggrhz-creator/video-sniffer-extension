...
// v4.4.2：单流代理（proxy-fetch-full）竞态兜底 —— SW 端停滞看门狗 30s，
// 但在 SW 被杀/消息通道断裂时发送端 Promise 永不 settle，页面无超时将
// 无限挂起（“下载卡住不动”）。15 分钟上限与 B站 downloadViaProxy 同款，
// 正常大文件永远跑不到，仅兜底异常通道。
const PROXY_FULL_RACE_MS = 15 * 60 * 1000;
...
const sendBgMessage = _NetHelpers.sendBgMessage || async function sendBgMessage(message, timeoutMs = 25000) {
  ...
  const raw = await Promise.race([
    ...
    new Promise((_, reject) => setTimeout(
      () => reject(new Error(t('de_sw_timeout', [timeoutMs / 1000, message?.type]))),
      timeoutMs,
    )),
  ]).catch((e) => {
  ...
  if (raw && typeof raw === 'object' && 'success' in raw) {
    return raw.success ? (raw.data ?? {}) : { error: raw.error || t('de_bg_failed') };
  }
  return raw;
};
...
class DownloadEngine {
  ...
  writeAt(position, data) {
    ...
    this.writeQueue = this.writeQueue.then(async () => {
      ...
      this._writeError = this._writeError || new Error(t('de_write_failed', [e?.message || e]));
    });
  ...
  async start() {
    ...
    if (!validateDownloadUrl(this.url)) {
      this.onError(new Error(t('de_url_invalid')));
    ...
    if (this._useProxy && typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
      try {
        const resp = await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error(t('de_probe_timeout'))), PROBE_RACE_TIMEOUT_MS)),
        ]);
  ...
  _validateMagic(bytes) {
    ...
    if (head4 === 'wOF2' || head4 === 'wOFF') {
      throw Object.assign(new Error(t('de_block_font')), { name: 'ContentError' });
    }
    if (b[0] === 0x3C) { // '<'
      throw Object.assign(new Error(t('de_block_html')), { name: 'ContentError' });
    }
    if (b[0] === 0x7B || b[0] === 0x5B) { // '{' '['
      throw Object.assign(new Error(t('de_block_json')), { name: 'ContentError' });
    }
    if (head4 === '%PDF') {
      throw Object.assign(new Error(t('de_block_pdf')), { name: 'ContentError' });
    }
    if (head4 === 'GIF8' || (b[0] === 0x89 && str(1, 3) === 'PNG')) {
      throw Object.assign(new Error(t('de_block_image')), { name: 'ContentError' });
    }
    if (head4.slice(0, 2) === 'PK') {
      throw Object.assign(new Error(t('de_block_zip')), { name: 'ContentError' });
    }
  ...
  fetchWithMode(url, opts = {}, timeout = FETCH_TIMEOUT_MS) {
    ...
    const timer = setTimeout(() => {
      controller.abort(new Error(t('de_req_timeout', [Math.round(timeout / 1000)])));
    }, timeout);
    const init = {
      mode: 'cors',
      redirect: 'follow',
  ...
  async _proxyFetch(url, opts = {}, timeout = FETCH_TIMEOUT_MS) {
    ...
    if (hasRange) {
      ...
      if (m) {
        const resp = await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error(t('de_proxy_seg_timeout', [Math.round(timeout / 1000)]))), timeout)),
        ]);
        if (resp?.error) throw new Error(resp.error);
        // 构造兼容的响应对象（兼容原有解析逻辑）
        return {
    ...
    const resp = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error(t('de_proxy_probe_timeout', [Math.round(timeout / 1000)]))), timeout)),
    ]);
    if (resp?.error) throw new Error(resp.error);
    const firstBytes = new Uint8Array(resp.firstBytes || []);
    return {
  ...
  async retryFailedPieces() {
    ...
    if (failed.length && !this.paused && !this.aborted) {
      throw new Error(t('de_segments_failed', [failed.length]));
    }
  }
  ...
  async _proxyFetchPiece(piece) {
    ...
    try {
      resp = await Promise.race([
        ...
        new Promise((_, reject) =>
          ...
          // 静默退出 → 下载永久停滞且零报错。
          setTimeout(() => reject(new Error(t('de_seg_proxy_timeout'))), SEGMENT_PROXY_TIMEOUT_MS)
        ),
      ]);
    } finally {
    ...
    if (resp?.error) {
      if (resp.error.includes('200') || resp.error.includes('Range')) {
        throw Object.assign(new Error(t('de_srv_no_range')), { name: 'NoRangeError' });
      }
    ...
    if (resp.actualStart !== null && resp.actualStart !== piece.start) {
      ...
      throw new Error(t('de_range_misalign', [piece.start, resp.actualStart]));
    }
    ...
    if (resp.opfsFile && this._useOPFS) {
      ...
      if (resp.opfsFile && !this._useOPFS) {
        throw new Error(t('de_opfs_unavailable'));
      }
    ...
    if (!isTailPiece && pieceBytes < piece.size) {
      throw new Error(t('de_seg_short_read', [pieceBytes, piece.size, piece.start]));
    }
  ...
  async _directFetchPiece(piece) {
    ...
    const checkStall = () => {
      if (Date.now() - lastDataTime > STALL_TIMEOUT_MS) {
        ...
        controller.abort(new Error(t('de_seg_stall', [STALL_TIMEOUT_MS / 1000])));
        return;
      }
    ...
    const fetchTimer = setTimeout(() => {
      controller.abort(new Error(t('de_seg_timeout', [FETCH_TIMEOUT_MS / 1000])));
    }, FETCH_TIMEOUT_MS);
    ...
    if (crHeader) {
      ...
      if (m) {
        ...
        if (actualStart !== piece.start) {
          try { await resp.body?.cancel(); } catch {}
          throw new Error(t('de_range_misalign', [piece.start, actualStart]));
        }
    ...
    if (pieceBytes < piece.size) {
      ...
      if (!fillsTailOrComplete) {
        throw new Error(t('de_seg_no_data', [pieceBytes, piece.size]));
      }
    }
  ...
  async _directFetchFull() {
    const controller = new AbortController();
    const timer = setTimeout(() => {
      controller.abort(new Error(t('de_req_timeout', [FETCH_TIMEOUT_MS / 1000])));
    }, FETCH_TIMEOUT_MS);
    try {
      const resp = await fetch(this.url, {
        mode: 'cors',
  ...
  async downloadSingleStream(source) {
    ...
    if (this._useProxy && !this._directBody && !source && typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
      let resp;
      try {
        // v4.4.2：单流代理竞态兜底 —— SW 被杀/消息通道断裂时不再无限挂起，
        // 15 分钟硬上限后转直连重试（正常大文件下载永远跑不到上限）
        resp = await Promise.race([
          sendBgMessage({
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('proxy-full-race-timeout')), PROXY_FULL_RACE_MS)),
        ]);
      } catch (e) {
        if (this.paused || this.aborted) return;
        console.warn('[VideoSniffer] 单流代理竞态超时，转直连:', e?.message || e);
        this._useProxy = false;
      }
      if (resp?.error) {
        if (this.paused || this.aborted) return;
        console.warn('[VideoSniffer] 单流代理失败，转直连:', resp.error);
        this._useProxy = false;
        ...
        try {
          ...
          // v4.4.2：完整性校验 —— SW 回传服务器 content-length。SW 被抓/
          // 断流时 OPFS 只剩半截；本地读取极快会“进度秒满但文件只有开头”。
          // 长度不足即视为失败（清 OPFS 后抛错，由上层走重试/直连），
          // 绝不静默保存损坏文件。用 `<` 而非 `!==`：响应被 gzip 等编码时
          // Chrome 解压后 body 可能比 content-length 长，属正常放行。
          const cl = resp.contentLength;
          if (cl && srcFile.size < cl) {
            console.warn(`[VideoSniffer] OPFS 文件不完整: ${resp.opfsFileName}, ${srcFile.size} bytes < content-length ${cl}`);
            try { await dir.removeEntry(resp.opfsFileName); } catch {}
            throw Object.assign(new Error(t('de_incomplete_download')), { code: 'INCOMPLETE' });
          }
          ...
          if (this.paused || this.aborted) return;
          // INCOMPLETE 是真正的完整性失败：清 OPFS 后转直连重下（不留半截）
          if (e?.code === 'INCOMPLETE') {
            this._useProxy = false;
          }
          console.warn('[VideoSniffer] 单流代理 OPFS 处理失败，转直连:', e?.message || e);
          this._useProxy = false;
        }
      }
    }
    ...
    while (true) {
      ...
      if (Date.now() - lastDataTime > FETCH_TIMEOUT_MS) {
        try { reader.cancel(); } catch {}
        throw new Error(t('de_conn_timeout'));
      }
      const { done, value } = await reader.read();
      if (done) break;
      lastDataTime = Date.now();
