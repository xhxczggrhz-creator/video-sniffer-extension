...
class StreamDownloader {
    ...
    try {
      ...
      if (skipped.length > 0 && !this.paused && !this.aborted) {
        ...
        if (this.isFMP4 || skipped.length > this.segments.length * 0.05) {
          throw new Error(
            `${skipped.length} 个分片反复下载失败（共 ${this.segments.length} 段），已中止，未生成损坏文件。建议减少并发任务数后重试`
          );
        }
  ...
  async processHLS(depth = 0) {
    if (depth > 5) throw new Error('播放列表嵌套过深');
    ...
    if (parsed.unsupportedEncryption) {
      throw new Error('此视频使用了不支持的加密方式（SAMPLE-AES），请使用录制模式');
    }
    ...
    if (parsed.encryption && parsed.encryption.method === 'AES-128') {
      ...
      if (!this.cryptoKey) {
        throw new Error('AES-128 密钥获取失败，请尝试录制模式');
      }
    }
  ...
  async processDASH() {
    ...
    if (doc.querySelector('parsererror')) throw new Error('DASH 清单解析失败');
    ...
    if (!bestRep) throw new Error('DASH 清单中没有视频轨道');
  ...
  async fetchText(url) {
    ...
    const timer = setTimeout(() => {
      controller.abort(new Error('清单获取超时（15 秒）——服务器响应过慢或网络受限，请重试或换一个条目/画质'));
    ...
    try {
      ...
      if (e instanceof TypeError) {
        ...
        if (real && !/代理返回空数据|清单请求超时$/.test(real)) {
          throw new Error(`清单获取失败：${real}`);
        }
        throw new Error('清单获取失败：服务器拒绝跨域访问或无响应。请重试，或回到列表换一个条目/画质下载');
      }
      throw e;
    } finally {
      clearTimeout(timer);
  ...
  async fetchBlob(url, segReqId) {
    ...
    if (this._useProxy && typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
      try {
        ...
        if (resp?.error) throw new Error(resp.error);
        if (!resp?.opfsFile) throw new Error('代理未返回 OPFS 文件句柄');
        if (!navigator.storage?.getDirectory) {
          throw new Error('本环境 OPFS 不可用，无法读取代理分片');
        }
        let blob;
        try {
          const root = await navigator.storage.getDirectory();
          ...
          if (resp.byteLength != null && blob.size !== resp.byteLength) {
            throw new Error(`分片长度校验失败（预期 ${resp.byteLength}，实际 ${blob.size}）`);
          }
        } finally {
  ...
  async _directFetchBlob(url) {
    const controller = new AbortController();
    const timer = setTimeout(() => {
      controller.abort(new Error('分片获取超时（30 秒）——服务器响应过慢或连接被中断'));
    ...
    try {
      ...
      if (e?.name === 'AbortError' || /aborted without reason/i.test(String(e?.message))) {
        throw new Error('分片获取超时（30 秒）——服务器响应过慢或连接被中断');
      }
      if (e instanceof TypeError) {
        throw new Error('分片获取失败：服务器拒绝跨域访问（Failed to fetch）');
      }
      throw e;
    } finally {
      clearTimeout(timer);
  ...
  async fetchEncryptionKey(keyUri) {
    ...
    try {
      ...
      const controller = new AbortController();
      const timer = setTimeout(() => {
        controller.abort(new Error('密钥获取超时（15 秒）'));
      }, MANIFEST_FETCH_TIMEOUT);
      let resp;
      try {
      ...
      if (keyBuf.byteLength !== 16) {
        throw new Error(`密钥长度异常: ${keyBuf.byteLength}`);
      }
  ...
  async decryptSegment(blob, seg) {
    ...
    if (seg.encryption.keyUri && seg.encryption.keyUri !== this.encryption?.keyUri) {
      ...
      if (!cryptoKey) throw new Error('分段密钥获取失败');
    }
