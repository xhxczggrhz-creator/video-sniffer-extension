...
```bash
npm run package     # → dist/video-sniffer-4.4.0.zip
```

已校验：

- 32 个文件，含 `_locales/zh_CN/messages.json` 与 `_locales/en/messages.json`
...
• 流媒体处理：HLS 分片（含 AES-128 解密）、DASH 自动选最高画质；TS 流下载完成后自动重封装为标准 MP4，失败自动降级，绝不产出坏文件
...
2. **搜索 + 排序**：搜索框输入关键词后的过滤结果（体现 v4.4.0 新功能）
3. **复制为命令**：展开某条的「⋯ 更多复制方式」菜单（curl / aria2c / ffmpeg）
4. **下载管理页**：多线程进度条 + 分段可视化 + 连接数 / 速度 / 剩余时间
5. **深色模式下的设置面板**（可选，展示国际化与主题）
